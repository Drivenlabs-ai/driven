...
| 8 | Contenu produit pour le travail (80 % des cas) | Markdown simple dans le dossier thématique | Brief client, positioning, deck, fiche produit, lead magnet, post draft, devis | Scope-check perso vs shared. Authors liste si shared. Pas de RULE factualité (c'est du contenu produit, pas une mémoire). Nature inférée : `status: wip` si étape de travail (ne fait pas foi) ; contextuel, jamais universel (`frontmatter.md` §contenu métier). |
